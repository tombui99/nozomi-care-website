...
const categories = ["Tin tức và Sự kiện", "Cẩm nang sức khoẻ", "Khuyến mãi"];
...
export const NewsEditor: React.FC = () => {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            setFormData({
              ...
              seoTags: Array.isArray(data.seoTags)
                ? data.seoTags.join(", ")
                : data.seoTags || "",
            });
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const blocks = tempDiv.querySelectorAll(
      ...
      const seoTags = formData.seoTags
        .split(",")
        .map((tag) => tag.trim())
        .filter(Boolean);
